...
# DEFAULT_DICT_PATH = "/Users/adrian/Documents/01_projects/02_clean_ppg/data/AURORA/preprocessed/data_dict_osc_auc_with_derivatives_with_classes.pt"
...
#         ax.set_title(label)
#         # for w in waves:
#         #     ax.plot(x, w, lw=0.8, color="gray", alpha=0.2)
#         # Plot mean and std
#         if waves:
#             mean_wave = np.mean(waves, axis=0)
#             std_wave = np.std(waves, axis=0)
#             TUM_BLUE = (0,101/255,189/255) # (0, 0.6, 1)
#             ax.plot(x, mean_wave, color=TUM_BLUE, lw=2, label='Mean')
#             ax.fill_between(x, mean_wave - std_wave, mean_wave + std_wave,
#                             color=TUM_BLUE, alpha=0.4, label='±1 Std Dev')
#             ax.legend(loc='upper right')

#         ax.set_xlim(0, 100)
#         ax.set_xlabel('Normalized time [%]')
#     axes[0].set_ylabel('Normalized PPG amplitude')
#     fig.tight_layout()
